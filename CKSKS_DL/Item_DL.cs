...
namespace ORS_RCM_DL
{
    public class Item_DL
    {
        ...
        public DataTable SearchItem(string ItemCode, string proname, string procode, string catInfo, string bname, string compname, string year, string season, string jan,int stock_flag, int pageindex, int pagesize, int option, Boolean isSearch)
        {
            try
            {
                if (isSearch)
                {
                    ...
                    if (stock_flag == 2)
                    {
                        da.SelectCommand.Parameters.AddWithValue("@stock_flag", DBNull.Value);
                    }
                    else
                    {
                        da.SelectCommand.Parameters.AddWithValue("@stock_flag", stock_flag);
                    }
