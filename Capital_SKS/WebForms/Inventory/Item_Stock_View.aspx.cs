    ...
    public partial class Item_Search : System.Web.UI.Page
    {
        ...
        protected Boolean IsSearch()
        {
            if (String.IsNullOrWhiteSpace(txtItemNumber.Text) && String.IsNullOrWhiteSpace(txtbrandname.Text) && String.IsNullOrWhiteSpace(txtcatinfo.Text) &&
               ...
               String.IsNullOrWhiteSpace(txtProductName.Text) && String.IsNullOrWhiteSpace(txtseason.Text) && String.IsNullOrWhiteSpace(txtyear.Text) && (ddlStockFlag.SelectedValue.ToString()=="2")
              )
            {
                return false;//for default all show
            }
            else return true;
        }
        ...
        protected DataTable Search(int page_index,int option)
        {
            ...
            int stock_flag = Convert.ToInt32(ddlStockFlag.SelectedValue.ToString());

            int page_size = Convert.ToInt32(ddlpage.SelectedValue.ToString());

            DataTable dt = itemBL.SearchItem(item_code, item_name, product, cat_info, brand_name, com_name, year, season, jan_code, stock_flag, page_index + 1, page_size, option,IsSearch());
