    ...
    public class Program
    {
        ...
        public static void LoginToYahooMallSetting()
        {
            try
            {
                ...
                string clientID = dt.Rows[0]["APIKey"].ToString();
                ...
                string url = "https://auth.login.yahoo.co.jp/yconnect/v2/authorization?response_type=code&client_id="+ clientID + "&redirect_uri=http://store.shopping.yahoo.co.jp/paintandtool/&scope=openid%20profile&state=xyz";
        ...
        public static string RedirectPath(string url)
        {
            try
            {
                ...
                using (IWebDriver firefox = new FirefoxDriver(option))
                {
                    ...
                    if (redirecturl.Contains("code"))
                    {
                        line = redirecturl.Substring(redirecturl.LastIndexOf('=') + 1);
                    }
                    firefox.Close();
                }
                return line;
