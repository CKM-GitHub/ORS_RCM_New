...
<script type="text/javascript">
    ...
    function UpdateQuantity(rowNo) {
        ...
        //get current id
        var ID = document.getElementById('MainContent_gvItem_txtID_' + rowNo).value;
        rowNo = parseInt(rowNo) + parseInt("1"); //get next rowNo that focus to another textbox
        var txt = document.getElementById('MainContent_gvItem_txtStockQuantity_' + rowNo);
        if (txt != null) {
            txt.focus(); //focus next textbox
...
<dl>
                    ...
                    <dt>在庫無限フラグ</dt>
					<dd>
                        <%--<asp:CheckBox ID="chkStockFlag" runat="server" Width="20px" Checked="true"></asp:CheckBox>--%>
                        <asp:DropDownList runat="server" ID="ddlStockFlag" style="margin-left:5px">
                            <asp:ListItem Value="2">全て</asp:ListItem>
                            <asp:ListItem Value="0">チェック</asp:ListItem>
                            <asp:ListItem Value="1">チェックを外す</asp:ListItem>
                        </asp:DropDownList>
                    </dd>
				</dl>
...
<asp:GridView runat="server" AutoGenerateColumns="False" ID="gvItem" 
		...
		onrowdatabound="gvItem_RowDataBound" onrowcommand="gvItem_RowCommand" PageSize="30" >
...
<asp:TextBox runat="server" ID="txtID" Text ='<%#Eval("ID") %>' style="display: none;"></asp:TextBox>
    <asp:Label runat="server" ID="lblID" Text ='<%#Eval("ID") %>' style="display: none;"></asp:Label>
</ItemTemplate>    
</asp:TemplateField>
...
<asp:TemplateField HeaderText="在庫無限フラグ">
<ItemTemplate>
