...
namespace API_R_Painttool
{
    public class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                Console.Title = "API Rakuten Painttool";
                ServicePointManager.Expect100Continue = true;
                ServicePointManager.SecurityProtocol = SecurityProtocolType.Tls12;//Tls12;
                ConsoleWriteLine_Tofile("API Rakuten Painttool : " + DateTime.Now);
                ...
                if (!string.IsNullOrWhiteSpace(list))
                {
                    ...
                    foreach (string itemcode in code)
                    {
                        ...
                        if (errCode1 != "C220" || errCode2 != "C302")
                        {
                            if (CtrlID.Equals("n"))
                            {
        ...
        static string GetXMl_NewUpdate(DataTable dtItem, DataTable dtInventory, DataTable dtCategory, DataTable dtOption, string itemCode, int mode)
        {
            ...
            writer.WriteStartElement("item");
            if(!String.IsNullOrWhiteSpace(dtItem.Rows[0]["Item_Code"].ToString()))
                writer.WriteElementString("itemUrl", dtItem.Rows[0]["Item_Code"].ToString());//Item_Code
            if (!String.IsNullOrWhiteSpace(dtItem.Rows[0]["Item_Code"].ToString()))
                writer.WriteElementString("itemNumber", dtItem.Rows[0]["Item_Code"].ToString());//商品番号
            if (!String.IsNullOrWhiteSpace(dtItem.Rows[0]["Item_Name"].ToString()))
                writer.WriteElementString("itemName", dtItem.Rows[0]["Item_Name"].ToString());//商品名
            if (!String.IsNullOrWhiteSpace(dtItem.Rows[0]["Sale_Price"].ToString()))
                writer.WriteElementString("itemPrice", dtItem.Rows[0]["Sale_Price"].ToString());//販売価格
            if (!String.IsNullOrWhiteSpace(dtItem.Rows[0]["Rakuten_CategoryID"].ToString()))
            {
            ...
            if (!String.IsNullOrWhiteSpace(dtItem.Rows[0]["Item_Description_PC"].ToString()))
            ...
            if (!String.IsNullOrWhiteSpace(dtItem.Rows[0]["Smart_Template"].ToString()))
                writer.WriteElementString("descriptionForSmartPhone", dtItem.Rows[0]["Smart_Template"].ToString());//スマートフォン用商品説明文
            if (!String.IsNullOrWhiteSpace(dtItem.Rows[0]["RF_Animation"].ToString()))
            ...
            if (!String.IsNullOrWhiteSpace(dtItem.Rows[0]["R_CatchCopy"].ToString()))
                writer.WriteElementString("catchCopyForPC", dtItem.Rows[0]["R_CatchCopy"].ToString());//PC用キャッチコピー
            if (!String.IsNullOrWhiteSpace(dtItem.Rows[0]["PC_CatchCopy_Mobile"].ToString()))
                writer.WriteElementString("catchCopyForMobile", dtItem.Rows[0]["PC_CatchCopy_Mobile"].ToString());//モバイル用キャッチコピー
            if (!String.IsNullOrWhiteSpace(dtItem.Rows[0]["Sale_Description_PC"].ToString()))
                writer.WriteElementString("descriptionBySalesMethod", dtItem.Rows[0]["Sale_Description_PC"].ToString());//PC用販売説明文
            if (!String.IsNullOrWhiteSpace(dtItem.Rows[0]["RF_Order_Button"].ToString()))
                writer.WriteElementString("isSaleButton", dtItem.Rows[0]["RF_Order_Button"].ToString());//注文ボタン
            if (!String.IsNullOrWhiteSpace(dtItem.Rows[0]["RF_Request_Button"].ToString()))
                writer.WriteElementString("isDocumentButton", dtItem.Rows[0]["RF_Request_Button"].ToString());//資料請求ボタン
            if (!String.IsNullOrWhiteSpace(dtItem.Rows[0]["RF_Product_Inquiry_Button"].ToString()))
                writer.WriteElementString("isInquiryButton", dtItem.Rows[0]["RF_Product_Inquiry_Button"].ToString());//商品問い合わせボタン
            if (!String.IsNullOrWhiteSpace(dtItem.Rows[0]["RF_Comingsoon_Button"].ToString()))
                writer.WriteElementString("isStockNoticeButton", dtItem.Rows[0]["RF_Comingsoon_Button"].ToString());//再入荷のお知らせボタン
            if (!String.IsNullOrWhiteSpace(dtItem.Rows[0]["RF_Product_Information"].ToString()))
                writer.WriteElementString("itemLayout", dtItem.Rows[0]["RF_Product_Information"].ToString());//商品情報レイアウト
            if (!String.IsNullOrWhiteSpace(dtItem.Rows[0]["RF_Consumption_Tax"].ToString()))
                writer.WriteElementString("isIncludedTax", dtItem.Rows[0]["RF_Consumption_Tax"].ToString());//消費税
            if (!String.IsNullOrWhiteSpace(dtItem.Rows[0]["R_Special_Flag"].ToString()))
                writer.WriteElementString("isIncludedPostage", dtItem.Rows[0]["R_Special_Flag"].ToString());//送料
            if (!String.IsNullOrWhiteSpace(dtItem.Rows[0]["Delivery_Charges"].ToString()))
                writer.WriteElementString("isIncludedCashOnDeliveryPostage", dtItem.Rows[0]["Delivery_Charges"].ToString());//代引き手数料
            if (!String.IsNullOrWhiteSpace(dtItem.Rows[0]["R_List_Price"].ToString()))
            ...
            if (!string.IsNullOrWhiteSpace(dtItem.Rows[0]["RF_Stock_Type"].ToString()))
                writer.WriteElementString("inventoryType", dtItem.Rows[0]["RF_Stock_Type"].ToString());//在庫タイプdtItem.Rows[0]["RF_Stock_Type"].ToString()
            #endregion
            ...
            if (!string.IsNullOrWhiteSpace(dtItem.Rows[0]["RF_Vertical_ItemName"].ToString()))
                writer.WriteElementString("verticalName", dtItem.Rows[0]["RF_Vertical_ItemName"].ToString());//項目選択肢別在庫用縦軸項目名
            if (!string.IsNullOrWhiteSpace(dtItem.Rows[0]["RF_Horizontal_ItemName"].ToString()))
            ...
            if (!string.IsNullOrWhiteSpace(dtItem.Rows[0]["Campaign_TypeID"].ToString()))
                writer.WriteElementString("asurakuDeliveryId", dtItem.Rows[0]["Campaign_TypeID"].ToString());//あす楽配送管理番号
            //writer.WriteElementString("deliverySetId", "2");//配送方法セット管理番号
            if (!string.IsNullOrWhiteSpace(dtItem.Rows[0]["RF_Size_Chartlink"].ToString()))
        ...
        static void CheckItemCodeForNewUpdate(string ItemList)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                HttpWebRequest webRequest = (HttpWebRequest)WebRequest.Create(string.Format("https://api.rms.rakuten.co.jp/es/1.0/item/get?itemUrl={0}", itemCode));
        ...
        static DataTable GetPromotionItemCode(string itemcode, string shopid, int option)
        {
            ...
            for (int i = 0; i < dtCategorylist.Rows.Count; i++)
            {
                ...
                try
                {
                    ...
                    if (response.StatusCode == HttpStatusCode.OK)
                    {
                        ...
                        if (list1[0].InnerText.Equals("C220") || list1[0].InnerText.Equals("C302"))
                        {
                            errCode1 = "C220";
                            errCode2 = "C302";
        ...
        {
            StreamWriter sw = new StreamWriter(consoleWriteLinePath + "API_RakutenPainttool.txt", true, System.Text.Encoding.GetEncoding("Shift_Jis"));
            sw.AutoFlush = true;
            Console.SetOut(sw);
